...
export class RealisticSimulatedPriceGenerator {
	...
	private async fetchRealMarketPrice(): Promise<number | null> {
		try {
			...
			logger.info(
				`Real market prices - Vast: $${vastPrice?.toFixed(4) || 'N/A'}, Akash: $${akashPrice?.toFixed(4) || 'N/A'}, Median: $${medianPrice.toFixed(4)}`,
			);
...
export const REALISTIC_DEMO_SCENARIOS = {
	...
	stable: {
		...
		fallbackBasePrice: 2.5, // Fallback if no real data
	},
