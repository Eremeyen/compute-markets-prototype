...
export class DemoController {
	...
	static switchScenario(scenario: keyof typeof REALISTIC_DEMO_SCENARIOS): void {
		...
		logger.info(
			`📊 Scenario config: ${JSON.stringify(REALISTIC_DEMO_SCENARIOS[scenario], null, 2)}`,
		);
	}
	...
	static listScenarios(): void {
		...
		Object.keys(REALISTIC_DEMO_SCENARIOS).forEach((scenario) => {
			const config =
				REALISTIC_DEMO_SCENARIOS[scenario as keyof typeof REALISTIC_DEMO_SCENARIOS];
			logger.info(
				`  • ${scenario}: ${(config.volatility * 100).toFixed(0)}% volatility, ${config.updateIntervalMs}ms updates`,
			);
		});
	}
	...
	static async runDemoSequence(): Promise<void> {
		...
		const scenarios: (keyof typeof REALISTIC_DEMO_SCENARIOS)[] = [
			'stable',
			'volatile',
			'bull',
			'bear',
	...
	static async showMarketStatus(): Promise<void> {
		...
		if (realMarketPrice !== null) {
			...
			const difference =
				simulatedPrice !== null
					? ((simulatedPrice - realMarketPrice) / realMarketPrice) * 100
					: 0;
