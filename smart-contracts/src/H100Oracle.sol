...
contract H100Oracle is Ownable {
    ...
    function updatePrice(uint256 _price, uint256 _ts, PriceSource _source) external onlyUpdaterOrOwner {
        if (_ts <= lastTimestamp) revert NonMonotonic();
        // Allow small clock skew: must not be more than 30s in the future relative to block time
        if (block.timestamp > _ts + 30) revert FutureTimestamp();
    ...
    function latestPriceWithMeta() external view returns (uint256 price, uint256 timestamp, PriceSource source) {
        if (block.timestamp - lastTimestamp > STALE_TOLERANCE) revert StaleOracle();
