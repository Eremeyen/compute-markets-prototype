...
contract DeployDemoSystemScript is Script {
	...
	function run() public {
		...
		console.log('To get 1000 USDC for testing:');
		console.log(
			'cast send ',
			address(mockUSDC),
			' "mintDemoAmount()" --private-key $PRIVATE_KEY'
		);
		console.log('');
		console.log('To check USDC balance:');
		console.log('cast call ', address(mockUSDC), ' "balanceOf(address)" ', deployer);
		console.log('');
		console.log('To check H100 price:');
		console.log('cast call ', address(oracle), ' "currentPrice()"');
		console.log('=== END DEMO COMMANDS ===');
	}
}
